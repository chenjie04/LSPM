...
import utils
from CNCF_base import ContextCF
...
def main():
    ...
    run_dir = "./run/cncf_base_taobao/{}".format(config['timestamp'])
    ...
    # Create model
    model = ContextCF(nb_users=nb_users, nb_items=nb_items, embed_dim=64)
    print(model)
    ...
    valid_results_file = os.path.join(run_dir, 'CNCF_base_taobao_valid_results.csv')
